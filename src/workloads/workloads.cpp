...
void Semaphore::consume(TimeDistribution &latency, uint32_t v, uint32_t wake) {
  ...
  while (true) {
    ...
    if (amount >= v) {
      ...
      int rc = syscall(SYS_futex, &count_, FUTEX_WAIT, cur, nullptr, nullptr, 0);
      if (rc != -EAGAIN) {
...
void Semaphore::produce(TimeDistribution &latency, uint32_t v, uint32_t wake) {
  uint32_t cur = count_.load(std::memory_order_acquire);
  while (true) {
    uint32_t amount = cur & ~(producer_waiter | consumer_waiter);
    if (amount + v <= max_) {
